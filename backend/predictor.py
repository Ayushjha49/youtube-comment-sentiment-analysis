...
class SentimentPredictor:
    ...
    def predict_ml(self, texts: List[str], cleaned: List[str] = None) -> np.ndarray:
        ...
        cleaned: pass pre-cleaned texts to skip redundant cleaning when
                 both ML and DL are running (shared cleaning optimisation).
        """
        self.load_ml()
        if cleaned is None:
    ...
    def predict_dl(self, texts: List[str], cleaned: List[str] = None) -> np.ndarray:
        ...
        cleaned: pass pre-cleaned texts to skip redundant cleaning.
        batch_size=256: 35 forward passes vs 139 at batch_size=64 for 8900 comments.
        """
        ...
        # 256 >> 64: ~4x fewer BiLSTM forward passes → ~2x faster DL inference
    ...
    def predict(
        ...
        if ml_ok and dl_ok:
            ...
            # Optimisation 1 — clean once, share between ML and DL
            # Without this: batch_clean runs twice on the same N comments.
            shared_cleaned = self._ml_cleaner.batch_clean(texts)

            # Optimisation 2 — run ML and DL in parallel
            # sklearn and TensorFlow both release the GIL during compute,
            # so threads genuinely overlap.
            # Sequential: ML(~15s) + DL(~25s) = ~40s
            # Parallel:   max(ML, DL)         = ~25s
